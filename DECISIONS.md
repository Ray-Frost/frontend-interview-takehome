...
- 后续为修复对齐问题引入 `offsetPx` 后，`BookingGrid` 又把整份 `visibleRange` 重新传给了 `RoomRow`。
- 由于 `offsetPx` 会随着横向拖动按像素连续变化，即使可见列窗口还没跨列，所有 `RoomRow` 也会在每一帧滚动里重新执行一次。
...
- 消息列表里的 `unread` 只来自 `/api/tickets` 返回值，点击 ticket 只会改 URL，不会改任何本地或服务端状态。
- 结果是进入具体会话后，列表里的蓝点和侧边栏未读数都不会减少，交互语义不完整。

###### 9. Booking 日历的日期基准仍然分散，存在 UTC / 时区偏移风险

- 当前 booking 日历把“今天”的基准分散在多个位置分别计算，并混用了 `toISOString().split('T')[0]` 与 `new Date('YYYY-MM-DD')` 这两条带 UTC 语义的日期处理链路。
- `mockData` 侧生成的 booking 日期、前端 `AppContext` 里的日历起止日，以及日历渲染层对 `YYYY-MM-DD` 的再次解析，并不保证始终基于同一个本地日期语义。
- 在非 UTC 时区或跨日边界下，这种实现存在 `1` 天偏移风险，可能表现为日历表头窗口、booking bar 位置和数据语义之间出现错位。
...
- 进一步把 `VisibleDayColumns` 和 `RoomRow` 的输入收窄为 `startIndex/endIndex` 这两个离散值，只让表头继续订阅连续变化的 `offsetPx`。
- 这样横向滚动时，表头仍能保持像素级对齐，而每一行只会在可见列窗口真的变化时重渲染，不会再被列内滑动带着整表一起跑。
...
###### 8. 在消息 provider 内维护会话级已读覆盖层

- 保持 `/api/tickets` 作为只读 mock 数据源，把“打开某条会话后应视为已读”放在 `MessagesProvider` 里处理。
- 当 URL 选中某条未读 ticket 时，provider 会把该 ticket 当前的 `updatedAt` 记录到本地读取快照里，并在派生 `tickets` 时仅对同一版本的会话覆盖 `unread` 为 `false`。
- 这样列表样式、右侧详情和侧边栏未读数都继续消费同一份 provider 数据，同时不需要额外引入一个临时写接口。
...
- 我把连续变化的 `offsetPx` 订阅限制在表头这一层，而不是继续向下透传到所有 `RoomRow`；这样做的代价是表头和行体使用的数据形状不完全一致，但它能更直接地把“像素级对齐”和“行级窗口渲染”这两类更新频率分开。
...
- 首屏进入 `/` 时，`Messages` 的 `badge` 会短暂处于未加载状态，这比错误显示 `0` 更符合真实语义。
...
- 把“本会话内已读”作为 provider 的局部 UI 状态维护，而不是直接改 `SWR cache`；因为在当前只读 `mock API` 下，单独改 `cache` 容易在后续 `revalidate` 时被原始数据冲回未读，也无法清晰表达“当前已读的是哪一个 `updatedAt` 版本”。
- 这种做法会让已读状态在刷新后回到 API 的初始值，但能稳定覆盖当前交互路径。
- 已读覆盖层额外绑定了 `updatedAt`，这样即使后续数据源变成真实后端，只要同一 ticket 收到更新并带来新的时间戳，本地已读覆盖也不会错误吞掉新的未读状态。

###### 9. Booking 日历的日期基准仍然分散，存在 UTC / 时区偏移风险

- 本次未直接修复，因为完整方案需要统一 API 输出的日历窗口、前端基准日来源和 `date-only` 计算方式，属于跨层的数据契约调整；当前先作为已知风险记录，后续再按单一来源收敛。
...
- 进一步把表头对齐用到的 `offsetPx` 从 React state / render 链路中拿掉，只保留 `startIndex` / `endIndex` 这类离散窗口信息进入组件树。
- 更具体地说，可以让 `useVisibleRange` 继续负责跨列时的窗口 state 更新，但把列内像素位移改为通过 `ref` 或 CSS variable 直接同步到表头 DOM，避免 `BookingGrid` 和表头层在每一帧横向滚动时都重新 render。
...
- 后续如果要完整修复，应先把 booking 日历的日期语义明确为“按房源所在地的本地日期解释”，而不是按浏览器本地日期或服务器本地日期解释。
- 日历窗口的基准日也应统一收口到 API 层，由 API 返回窗口起止日或等价的日历范围；前端只消费这份结果，不再自行计算“今天”或窗口边界。
- 同时应把 `YYYY-MM-DD` 明确定义为 `date-only` 业务值，而不是时间点；后续日期加减、天数偏移和 label 构建都通过同一套 `date-only helper` 处理，避免继续混用 `toISOString().split('T')[0]` 和 `new Date('YYYY-MM-DD')`。
- 这样可以让 booking / availability / occupancy 这类日历稳定对应房源当地的那一天，并减少跨日边界、时区差异或缓存场景下表头窗口、booking bar 和数据语义之间的错位风险。
